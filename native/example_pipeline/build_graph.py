...
parser.add_argument('--partition_points', '-p', nargs='+',
                    type=str, default=[], help='set partition points')
parser.add_argument('--img_size', '-i', type=int,
                    default=224, help='set image size')
# parser.add_argument('--model', '-m', type=str, default='unet', help='name of model')
parser.add_argument('--model', '-m', type=str,
                    default='resnet152', help='name of model')
parser.add_argument('--target', '-t', type=str,
                    default='cuda', help='name of taget')
# parser.add_argument('--device', '-d', type=str, default='2080ti', help='name of device')
parser.add_argument('--opt_level', '-o', type=int,
                    default=4, help='set opt_level')
parser.add_argument('--build', '-b', type=int, default=0, help='build model')
parser.add_argument('--quantization_level', '-q', type=int,
                    default=2, help='set quantization level')
parser.add_argument('--model_config', '-c', nargs='+',
                    type=int, default=0, help='set partition point')
parser.add_argument('--jetson', '-j', type=int, default=0, help='jetson')
parser.add_argument('--base_path', type=str,
                    default=os.environ['TS_DATA_PATH'] + "/model_tf/unet_v1/best/", help='path setting')
# parser.add_argument('--tvm_build_path', type=str, default=os.environ['TS_DATA_PATH'] + "/model_tvm/unet_v1/best/", help='path setting')
parser.add_argument('--tvm_build_path', type=str,
                    default=os.environ['TS_DATA_PATH'] + "/model_tvm/resnet50/", help='path setting')
parser.add_argument('--gpu_index', type=str, default="0",
                    help='set target gpu index')
...
if args.model == 'resnet50':
    ...
    model_keras = tf.keras.models.load_model(
        '/'.join([base_path, "UNet_M[{}-{}-{}-{}].h5"]).format(*model_config))
    # tvm result
    input_data = input_data.transpose([0, 3, 1, 2])
    shape_dict = {"input_1": input_data.shape}
...
if is_jetson == 1:
    ...
    gpu_info = subprocess.check_output(
        "nvidia-smi --query-gpu=index,name --format=csv", shell=True, encoding='utf-8').split("\n")[1:-1]
    gpu_info = {
        x.split(', ')[0]: x.split(', ')[1]
        for x in gpu_info
    }
...
model_info_format = "{name}-{host}-{target}-{arch}-o{opt}-q{qaunt}-s{slice}".format(
    **model_info)
...
if args.build == 1:
    ...
    if is_jetson == 1:
        param_format = '/'.join([tvm_build_path,
                                model_info_format + ".params"])
    else:
        param_format = '/'.join([tvm_build_path,
                                model_info_format + ".params"])
...
json_format = '/'.join([tvm_build_path, model_info_format +
                       "{}.json"]).format(slice_points)
# print([1], [len(graph_info['nodes'])-1])
graph_json, input_indexs, output_indexs = tvm_slicer.slice_graph(
    [1], [len(graph_info['nodes'])-1], is_quantize_sliced=True)
graph_json['extra'] = {}
