...
parser.add_argument('--base_path', type=str, default=".", help='path setting')
args = parser.parse_args()

data_path = os.environ.get("TS_DATA_PATH", "")
raw_data_path = data_path + "/us-bmod/{}/raw/0/"
mask_data_path = data_path + "/us-bmod/{}/mask/0/"
...
def get_model_info(partition_points):
    ...
    for i in range(len(partition_points) - 1):
        ...
        with open('/'.join([target_path, "UNet_M[{}-{}-{}-{}]_Q[{}]_S[{}-{}].json"]).format(
            *model_config, 
            quantization_level, 
...
if __name__ == '__main__':
    ...
    # Load models
    model_path = '/'.join([target_path, "UNet_M[{}-{}-{}-{}]_Q[{}]_full.so"]).format(*model_config, quantization_level)
    # model_path = "../src/model/{}_{}_full_{}_{}.so".format(args.model, args.target, args.img_size, args.opt_level)
    lib = tvm.runtime.load_module(model_path)

    param_path = '/'.join([target_path, "UNet_M[{}-{}-{}-{}]_Q[{}]_full.params"]).format(*model_config, quantization_level)
