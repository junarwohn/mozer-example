...
def load_data():
    cap = cv2.VideoCapture("/".join([base_path, "j_scan.mp4"]))
...
def get_model_info(partition_points):
    ...
    for i in range(len(partition_points) - 1):
        ...
        if is_jetson == 1:
            json_format = "/".join([base_path, "tf_model/unet_v1/best", "UNet_M[{}-{}-{}-{}]_Q[{}]_S[{}-{}]_jetson.json"])
        else:
            json_format = "/".join([base_path, "tf_model/unet_v1/best", "UNet_M[{}-{}-{}-{}]_Q[{}]_S[{}-{}].json"])
...
if __name__ == '__main__':
    ...
    # Load models
    if is_jetson == 1:
        model_format = "/".join([base_path, "tf_model/unet_v1/best", "UNet_M[{}-{}-{}-{}]_Q[{}]_full_jetson.so"])
    else:
        model_format = "/".join([base_path, "tf_model/unet_v1/best", "UNet_M[{}-{}-{}-{}]_Q[{}]_full.so"])
    ...
    if is_jetson == 1:
        param_format = "/".join([base_path, "tf_model/unet_v1/best", "UNet_M[{}-{}-{}-{}]_Q[{}]_full_jetson.params"])
    else:
        param_format = "/".join([base_path, "tf_model/unet_v1/best", "UNet_M[{}-{}-{}-{}]_Q[{}]_full.params"])
