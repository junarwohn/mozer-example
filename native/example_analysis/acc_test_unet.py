...
def acc_test(lib, dev, loaded_params, input_idxs, output_idxs, graph_json_strs):
    ...
    for input_idx, output_idx, graph_json_str in zip(input_idxs, output_idxs, graph_json_strs):
        ...
        for file_path in file_list[:100]:
            ...
            data = np.expand_dims(img, 0).transpose([0, 3, 1, 2]) / 255
            dtype = "float32"
            model.set_input("input_{}".format(0), tvm.nd.array(data.astype(dtype)))
            model.run()
            tvm_out = model.get_output(0).numpy()
            th = cv2.resize(cv2.threshold(np.squeeze(tvm_out.transpose([0,2,3,1])), 0.5, 1, cv2.THRESH_BINARY)[-1], (img_size,img_size))

            output_log.append(th)   
        np.save("./vco_unet_q0.npy", np.array(output_log))
...
#  
raw_input_dummy = np.random.normal(0, 1, (1,256,256,3))
raw_output_dummy = np.random.normal(0, 1, (1,256,256,1))
